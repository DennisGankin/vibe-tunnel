...
- **Memory**: the job log's `allocated:` line shows what slurm granted. A first launch into a fresh sandbox home peaks around 1 GB (native Claude install); `submit` warns below 1 GB total, and all inputs require a unit on memory values.
- **Dev Containers / Remote-WSL / Remote-SSH** are pinned to the local side via `<sandbox home>/.vscode-server/data/Machine/settings.json` (`remote.extensionKind`), written on first start of a home; existing homes get the key merged in when `jq` is in the image.
