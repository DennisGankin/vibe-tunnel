...
DO_CLI=1 DO_PATH=1 FORCE=0 YES=0
...
# --- 4. PATH -----------------------------------------------------------------------
step "PATH"
if [ "$DO_PATH" -eq 0 ]; then say "  ${DIM}skipped (--no-path)${RST}"
else
...
say "  …or from your laptop:    ${CYAN}python vibe_tunnel.py${RST}"
